...
def fit_weighted_average(
    ...
    return {"accuracy": sum(accuracies) / sum(examples), "loss": sum(losses) / sum(examples), "loss_distributed": sum(losses_distributed) / sum(examples), "precision": sum(precisions) / sum(examples), "recall": sum(recalls) / sum(examples), "f1_score": sum(f1_scores) / sum(examples)}
